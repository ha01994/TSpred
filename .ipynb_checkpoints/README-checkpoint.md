...
The above code will return predictions in the `ensemble_predictions.csv` file. A score of 0 means that a pair is predicted to be non-binding and 1 means that a pair is predicted to bind. Therefore, the higher the score, the more likely a given TCR-peptide pair will bind, according to our model.
